...
Use different button types to build hierarchy in an application layout.
  ...
  **Icon button**
  Use icon buttons for actions that are commonly associated with icons. For example, you could use the close icon to exit a modal or window.
...
**External links**
For links that navigate users to any location external to the current website or application, link text must be followed by the [external link icon](/design-guidelines/styles/icons).

![External link icon](img/external-link.png)

When possible, external links should open in a new browser tab or window, depending on the user's browser preferences.
